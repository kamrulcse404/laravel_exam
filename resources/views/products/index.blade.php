                                    ...
                                    <dl class="row mb-0" style="height: 80px; overflow: hidden" id="variant">
                                        @foreach ($product->product_variant_prices as $price)
                                            <dt class="col-sm-3 pb-0">
                                                {{ strtoupper($price->productVariantOne->variant) ?? '' }} /

                                                @if (empty($price->productVariantThree))
                                                    {{ ucfirst($price->productVariantTwo->variant) ?? '' }}
                                                @else
                                                    {{ ucfirst($price->productVariantTwo->variant) ?? '' }} /
                                                @endif

                                                @if (!empty($price->productVariantThree->variant))
                                                    {{ ucfirst($price->productVariantThree->variant) ?? '' }}
                                                @endif
                                            ...
                                            <dd class="col-sm-9">
                                                <dl class="row mb-0">
                                                    <dt class="col-sm-4 pb-0">Price : {{ number_format($price->price, 2) }}
                                                    </dt>
